...
def get_available_slots():
    ...
    if entity_id == 'faculty' and doctor_name:
        cursor.execute('''
            SELECT booking_time FROM bookings 
            WHERE target_entity = ? AND department = ? AND booking_date = ? AND doctor_name = ?
        ''', (entity_id, department, booking_date, doctor_name))
    else:
        cursor.execute('''
            SELECT booking_time FROM bookings 
...
def book():
    ...
    if entity_id == 'faculty':
        cursor.execute('''
            ...
            WHERE target_entity = ? AND department = ? AND booking_date = ? AND booking_time = ? AND doctor_name = ?
        ''', (entity_id, department, booking_date, booking_time, doctor_name))
    else:
        cursor.execute('''
            SELECT id FROM bookings 
    ...
    if cursor.fetchone():
        conn.close()
        flash('عذراً، هذا الوقت تم حشزه للتو عند الموظف المختار! يرجى اختيار وقت آخر.', 'danger')
        return redirect(url_for('select_time', entity_id=entity_id))
        
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    cursor.execute('''
        INSERT INTO bookings (trainee_name, trainee_id, department, target_entity, doctor_name, booking_date, booking_time, timestamp)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ''', (trainee_name, trainee_id, department, entity_id, doctor_name, booking_date, booking_time, timestamp))
    ...
    session['success_info'] = {
        ...
        'entity': TARGET_ENTITIES[entity_id] + (f" ({doctor_name})" if doctor_name else ""),
        'date': booking_date,
        'time': booking_time
    }
